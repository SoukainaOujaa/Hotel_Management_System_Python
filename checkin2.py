...
def checkinn2():
    ...
    insertR = "Insert into checkin(LNAME,Rnb,Date,Nbr,P) values('"+Lname+"','"+Rnb+"','"+Date+"','"+Nbr+"','"+Pay+"')"
    try:
        cur.execute(insertR)
        con.commit()
        messagebox.showinfo('Success',"Réservée avec succès")
        updateroom="update rooms set status='booked'where RoomNo='"+Rnb+"'"
        cur.execute(updateroom)
        con.commit()
        messagebox.showinfo('Success',"Statut de chambre modifié")
    except:
        messagebox.showinfo("Error","Existe déjà")
...
def checkin2():
    ...
    headingLabel = Label(headingFrame1, text="Réservation", bg='black', fg='white', font=('Courier',15))
    ...
    #Room number
    lb2 = Label(labelFrame,text="N° chambre : ", bg='black', fg='white')
    ...
    #Submit Button
    SubmitBtn = Button(root3,text="Ajouter",bg='#47290F', fg='white',command=checkinn2)
